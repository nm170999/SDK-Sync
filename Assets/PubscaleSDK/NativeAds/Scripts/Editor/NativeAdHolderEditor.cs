...
namespace PubScale.SdkOne.NativeAds
{
    ...
    public class NativeAdHolderEditor : Editor
    {
        ...
        protected SerializedProperty Prop_AdResizeScale;
        //protected SerializedProperty Prop_AdFrameBadge;
        ...
        public void OnEnable()
        {
            ...
            Prop_AdResizeScale = serializedObject.FindProperty(nameof(targetHolder.AdResizeScale));
            // Prop_AdFrameBadge = serializedObject.FindProperty(nameof(targetHolder.AdBadgeFrame));
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                PubEditorUX.DisplayFloat(Prop_AdResizeScale, new GUIContent("AD RESIZE SCALE: "));
                // PubEditorUX.DisplayProperty(Prop_AdFrameBadge, new GUIContent("AD BADGE FRAME: "), GUILayout.MaxWidth(600));

                float resizeScale = Prop_AdResizeScale.floatValue;

                if (resizeScale < PubScaleConstants.MinResizeScale)
                {
                    Debug.LogWarning("A minimum scale of atleast " + PubScaleConstants.MinResizeScale + "s is required for ad tag " + targetHolder.adTag);
                    EditorGUILayout.Space();
                    EditorGUILayout.Space();
                    Prop_AdResizeScale.floatValue = PubScaleConstants.MinResizeScale;
                }
            }
